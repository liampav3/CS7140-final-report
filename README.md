...
where $L$ is the number of layers and $K_i$ is the dimensionality of the $i$-th layer. This representation corresponds to some base weight matrix $M_i$ having its features randomly dropped out with probability $p_i$. The base matrices $M_i$ and the dropout probabilities $p_i$ are optimized to minimize an approximation of KL divergence with the true posterior \[5\].
...
which corresponds to averaging the models output over $T$ different runs with independently drawn dropout patterns. The variance of the model at this point is measured as
...
For classification, the networks are trained using a proper scoring rule since these reward accurate predictive distributions. Examples of proper scoring rules include negative log loss and Brier score. For regression, the networks are configured to output two values, a predicted mean $\mu$ and predicted variance $\sigma^2$, and trained to minimize the loss function

$$ \frac{\log \sigma^2(x)}{2} + \frac{(y-\mu(x))^2}{2\sigma^2(x)} $$

where $y$ is the value at input $x$ from the training data \[9\].
